...
import { Save, FileText, Printer, Filter, Calendar, ShoppingCart } from "lucide-react";
...
/** If InsertTreatment doesn't include encounterId, extend it locally for the save */
type SaveTreatmentPayload = InsertTreatment & { encounterId?: string };
...
export default function Treatment() {
  ...
  useEffect(() => {
    ...
    if (!hasConsult && services.length > 0) addConsultationMutation.mutate();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentEncounter?.encounterId, orderLines.length, orders.length, services.length]);
  ...
  const createTreatmentMutation = useMutation({
    mutationFn: async (data: SaveTreatmentPayload): Promise<Treatment> => {
  ...
  const handleCloseVisit = () => {
    ...
    const hasDiagnosis =
      (persistedDiagnosis && persistedDiagnosis.trim() !== "") ||
          ...
          <CardContent>
            <div className="space-y-3">
              {todaysTreatments && todaysTreatments.length > 0 ? (
                ...
                <p className="text-gray-500 dark:text-gray-400 text-center py-4">No treatment visits recorded today</p>
              )}
            </div>
          </CardContent>
        </Card>
                ...
                <div className="p-4 bg-white dark:bg-gray-900 rounded-lg border">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        ID: {selectedPatient.patientId} | Age: {getAge(selectedPatient.age || "")}{" "}
                    ...
                    <div className="text-right">
                      ...
                      <Button variant="outline" size="sm" className="ml-2" onClick={() => setSelectedPatient(null)}>
                        Change
                      </Button>
                    </div>
                  </div>
              ...
              <Card className="border-l-4 border-l-blue-500 mb-6">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    {orders.some(
                      (o) => o.status === "completed" && !o.orderLine?.addToCart && !!o.orderLine?.acknowledgedBy
                    ) && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          const toAdd = orders.filter(
                            (o) =>
                              o.status === "completed" &&
                              !o.orderLine?.addToCart &&
                              !!o.orderLine?.acknowledgedBy
                          );
                          toAdd.forEach((order) => {
                            if (order.orderLine?.id) {
                              addToCartMutation.mutate({
                                orderLineId: order.orderLine.id,
                                addToCart: true,
                              });
                            }
                          });
            ...
            {selectedPatient && currentEncounter && (
              <Card className="border-l-4 border-l-green-500 mb-6">
                  ...
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary">
                        {orders.filter((o) => !!o.orderLine?.addToCart).length} items
                      </Badge>
                      <Button variant="outline" size="sm" onClick={() => setShowVisitCart(!showVisitCart)}>
                ...
                {showVisitCart && (
                  <CardContent>
                    <div className="space-y-3">
                      {orders.filter((o) => !!o.orderLine?.addToCart).length === 0 ? (
                        ...
                        <>
                          {orders
                            .filter((o) => !!o.orderLine?.addToCart)
                            .map((order: any) => (
                              <div
                                key={order.orderLine?.id ?? order.orderId}
                              ...
                              >
                                <div>
                                  ...
                                  {order.flags && (
                                    <Badge
                                      variant={order.flags === "critical" ? "destructive" : "outline"}
                                      className="mt-1"
                                    >
                                      {order.flags}
                                    </Badge>
                          ...
                          <div className="border-t pt-3 flex justify-between items-center">
                            ...
                            <span className="font-bold text-lg text-green-600 dark:text-green-400">
                              {orders.filter((o) => !!o.orderLine?.addToCart).length} service(s)
                            </span>
                          </div>
                        </>
                      )}
            ...
            <CardContent className="p-6">
              ...
              >
                ...
                <div className="text-center border-b pb-4 mb-6">
                  ...
                  <p className="text-lg font-semibold text-medical-green mt-2">PRESCRIPTION</p>
                </div>
