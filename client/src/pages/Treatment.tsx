                            ...
                            <div>
                              ...
                              <div className="space-y-3">
                                {labTests.filter((t: any) => t.status === "completed").map((test: any) => {
                                  ...
                                  // Extract key finding from results
                                  const getKeyFinding = () => {
                                    // Check for malaria parasites
                                    const bffm = parsedResults["Blood Film for Malaria (BFFM)"];
                                    if (bffm && bffm["Malaria Parasites"] && bffm["Malaria Parasites"] !== "Not seen" && bffm["Malaria Parasites"] !== "Negative") {
                                      return `Positive for ${bffm["Malaria Parasites"]} malaria – requires immediate treatment.`;
                                    }
                                    
                                    // Check for severe anemia
                                    const cbc = parsedResults["Complete Blood Count (CBC)"];
                                    if (cbc && cbc["Hemoglobin"]) {
                                    ...
                                    // Check for positive infections
                                    const hbsag = parsedResults["Hepatitis B Test (HBsAg)"];
                                    if (hbsag && (hbsag["HBsAg"] === "Reactive" || hbsag["HBsAg"] === "Positive")) {
                                      return "Hepatitis B positive – patient is infectious.";
                                    }
                                    
                                    const hiv = parsedResults["HIV Test"];
                                    if (hiv && hiv["HIV Antibody"] === "Positive") {
                                      return "HIV positive – requires confirmatory testing and counseling.";
                                    }
                                    
                                    // Default: No critical findings
                                    return null;
                                  };
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      {/* Key Finding Preview */}
                                      {keyFinding && (
                                        <div className="mx-4 mb-3 p-3 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-md">
