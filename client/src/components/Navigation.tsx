...
export default function Navigation({ isMobileOpen = false, onMobileClose }: NavigationProps = {}) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed left-0 top-0 h-full w-64 bg-white border-r border-gray-200 shadow-lg dark:bg-gray-900 dark:border-gray-700 pt-16 z-50 transition-transform duration-300
        ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* Close button for mobile */}
        <button
          onClick={onMobileClose}
          className="absolute top-20 right-4 lg:hidden p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        ...
        <nav className="h-full overflow-y-auto px-4 py-6 pb-24">
                ...
                {items.map((item) => {
                  ...
                  return (
                    <Link key={item.path} href={item.path}>
                      <div 
                        className={`
                          group flex items-center px-3 text-sm font-medium rounded-xl transition-all duration-200 cursor-pointer min-h-[44px]
                          ${isActive 
                          ...
                          focus-within:ring-2 focus-within:ring-medical-blue focus-within:ring-offset-2
                        `}
