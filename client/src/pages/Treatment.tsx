...
export default function Treatment() {
  ...
  const acknowledgeMutation = useMutation({
    mutationFn: async ({
      ...
      const r = await apiRequest("PUT", `/api/order-lines/${orderLineId}/acknowledge`, {
        acknowledgedBy,
        acknowledged,
      });
      return r.json();
  ...
  useEffect(() => {
    ...
    // include orders in deps to avoid multiple posts if still loading; JSON.stringify is fine here
    ...
    <div className="space-y-6">
      <Card className="print:hidden">
        ...
        <CardContent>
          ...
          {/* ✅ OmniOrderBar lives here (only when an encounter exists) */}
          {selectedPatient && currentEncounter && (
            <OmniOrderBar
              encounterId={currentEncounter.encounterId}
              services={services}
                ...
                <CardHeader className="pb-3">
                  <CardTitle
                    className={`flex items-center gap-2 text-base ${
                      selectedPatient.allergies && selectedPatient.allergies.trim() ? "text-red-700 dark:text-red-400" : ""
                    }`}
                  >
                    <AlertTriangle
                      className={`h-5 w-5 ${
                        selectedPatient.allergies && selectedPatient.allergies.trim() ? "text-red-600" : "text-gray-400"
                      }`}
                    />
                    Allergies
                  </CardTitle>
                  ...
                  {recentTreatments.length > 0 ? (
                    <div className="space-y-2">
                      {recentTreatments.map((t) => (
                        <div key={t.treatmentId} className="bg-gray-50 dark:bg-gray-800 p-2 rounded text-sm border-l-2 border-blue-400">
                          ...
                          {t.diagnosis && (
                            <p className="text-gray-700 dark:text-gray-300 text-xs">
                              <span className="font-medium">Dx:</span> {t.diagnosis}
                            </p>
                          )}
                        </div>
                      ))}
                    ...
                    <div className="space-y-4">
                                        ...
                                        <>
                                          ...
                                          {test.orderLine.acknowledgedBy && !test.orderLine.addToCart && (
                                            <Button
                                              ...
                                              size="sm"
                                              onClick={() =>
                                                addToCartMutation.mutate({ orderLineId: test.orderLine.id, addToCart: true })
                                              }
                      ...
                      {xrays.length > 0 && (
                        <div>
                          ...
                          <div className="space-y-2">
                            {xrays.map((x: any) => (
                              <div key={x.examId || x.orderId} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                <div className="flex justify-between items-start gap-3">
                                  <div className="flex-1">
                                    ...
                                    {x.status === "completed" && x.orderLine && (
                                      <>
                                        <div className="flex items-center gap-2">
                                          ...
                                          />
                                          <label htmlFor={`ack-xray-${x.id}`} className="text-sm cursor-pointer">
                                            Acknowledge
                                          </label>
                                        </div>
                                        {x.orderLine.acknowledgedBy && !x.orderLine.addToCart && (
                                          <Button
                                            ...
                                            size="sm"
                                            onClick={() =>
                                              addToCartMutation.mutate({ orderLineId: x.orderLine.id, addToCart: true })
                                            }
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {ultrasounds.map((u: any) => (
                              <div key={u.examId || u.orderId} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                <div className="flex justify-between items-start gap-3">
                                  <div className="flex-1">
                                    ...
                                    {u.status === "completed" && u.orderLine && (
                                      <>
                                        <div className="flex items-center gap-2">
                                          ...
                                          />
                                          <label htmlFor={`ack-ultrasound-${u.id}`} className="text-sm cursor-pointer">
                                            Acknowledge
                                          </label>
                                        </div>
                                        {u.orderLine.acknowledgedBy && !u.orderLine.addToCart && (
                                          <Button
                                            ...
                                            size="sm"
                                            onClick={() =>
                                              addToCartMutation.mutate({ orderLineId: u.orderLine.id, addToCart: true })
                                            }
                        ...
                        {orders.filter((o: any) => o.addToCart).length === 0 ? (
                          ...
                          <>
                            {orders
                              ...
                              .map((o: any) => (
                                <div key={o.orderId} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                  <div>
                                    ...
                                    {o.flags && (
                                      <Badge variant={o.flags === "critical" ? "destructive" : "outline"} className="mt-1">
                                        {o.flags}
                                      </Badge>
                                    )}
                                  </div>
                                  <div className="text-right">
                                    {o.orderLine?.id && (
                                      <Button
                                        ...
                                        size="sm"
                                        onClick={() =>
                                          addToCartMutation.mutate({ orderLineId: o.orderLine.id, addToCart: false })
                                        }
                                      >
                                        Remove
                                      </Button>
                                      ...
                                      >
                                        <div className="flex items-start justify-between">
                                          <div className="flex-1">
                                            ...
                                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                              Order ID: {rx.orderId} | Prescribed:{" "}
                                    ...
                                    {medications.map((med, idx) => (
                                      <div key={idx} className="flex items-start justify-between p-3 bg-white dark:bg-gray-900 border rounded-lg">
                                        <div className="flex-1">
                                          ...
                                          {med.instructions && (
                                            <p className="text-sm text-gray-500 dark:text-gray-500">Instructions: {med.instructions}</p>
                                          )}
                                        </div>
                                        <Button
                                          type="button"
      ...
      {/* Prescription print sheet (unchanged rendering; appears only when showPrescription is true) */}
