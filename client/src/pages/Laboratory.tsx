...
const getClinicalInterpretation = (results: string) => {
  ...
  try {
    ...
    if (parsed['HIV Test (RCT P24)']) {
      ...
      if (hiv['HIV P24 Antigen']?.includes('Positive')) {
        findings.push('🚨 HIV positive - Requires immediate specialist consultation and counseling');
      }
    }
    
    // Check Liver Function Test (LFT)
    if (parsed['Liver Function Test (LFT)']) {
      const lft = parsed['Liver Function Test (LFT)'];
      const alt = parseFloat(lft['ALT (SGPT)']);
      const ast = parseFloat(lft['AST (SGOT)']);
      const bilirubin = parseFloat(lft['Bilirubin Total']);
      const alkp = parseFloat(lft['Alkaline Phosphatase']);
      
      // Check for elevated liver enzymes
      if (alt > 56) {
        findings.push('🚨 Elevated ALT - Liver damage or inflammation detected');
      }
      if (ast > 40) {
        findings.push('🚨 Elevated AST - Liver or muscle damage detected');
      }
      if (bilirubin > 1.2) {
        findings.push('🚨 Elevated bilirubin - Liver dysfunction or hemolysis');
      }
      if (alkp > 147) {
        findings.push('⚠️ Elevated alkaline phosphatase - Liver or bone disease');
      }
      
      // Add normal liver function note if all values are normal
      if (alt <= 56 && ast <= 40 && (isNaN(bilirubin) || bilirubin <= 1.2) && (isNaN(alkp) || alkp <= 147)) {
        findings.push('✅ Liver function tests normal - Good hepatic status');
      }
    }
    
    // Check Renal Function Test (RFT)
    if (parsed['Renal Function Test (RFT)']) {
      ...
      if (urea > 20) {
        findings.push('🚨 Elevated urea - Kidney dysfunction detected');
      }
      if (creatinine > 1.2) {
        findings.push('🚨 Elevated creatinine - Reduced kidney function');
      }
      if (bun > 23) {
        findings.push('⚠️ Elevated BUN - Kidney function assessment needed');
      }
      
      // Add normal kidney function note if all values are normal
      if (urea <= 20 && creatinine <= 1.2 && (isNaN(bun) || bun <= 23)) {
        findings.push('✅ Kidney function tests normal - Good renal status');
      }
    }
    ...
    if (parsed['Blood Sugar (RBS/FBS)']) {
      ...
      const glucose = parseFloat(sugar['Glucose Level']);
      
      if (glucose > 140) {
        findings.push('🚨 Elevated blood glucose - Diabetes screening required');
      } else if (glucose < 70) {
        findings.push('🚨 Low blood glucose - Hypoglycemia detected');
      }
    }
    ...
    if (parsed['Hemoglobin (HB)']) {
      ...
      const hemoglobin = parseFloat(hb['Hemoglobin Level']);
      
      if (hemoglobin < 6.0) {
        findings.push('🚨 CRITICALLY LOW hemoglobin - Severe anemia requires immediate transfusion');
      } else if (hemoglobin < 10.0) {
        findings.push('🚨 LOW hemoglobin - Moderate to severe anemia detected');
      } else if (hemoglobin < 12.0) {
        findings.push('⚠️ Mild anemia detected - Iron supplementation may be needed');
      }
    }
    ...
    if (parsed['Complete Blood Count (CBC)']) {
      ...
      const wbc = parseFloat(cbc['WBC Count']);
      const rbc = parseFloat(cbc['RBC Count']);
      const platelets = parseFloat(cbc['Platelet Count']);
      
      if (wbc > 11000) {
        findings.push('🚨 Elevated WBC - Active infection or inflammation');
      } else if (wbc < 4000) {
        findings.push('⚠️ Low WBC - Immune system compromise');
      }
      
      if (platelets < 150000) {
        findings.push('🚨 Low platelets - Bleeding risk, avoid surgery');
      }
    }
    
    // Check Thyroid Function
    if (parsed['Thyroid Function Test (T3, T4, TSH)']) {
      const thyroid = parsed['Thyroid Function Test (T3, T4, TSH)'];
      const tsh = parseFloat(thyroid['TSH']);
      const t3 = parseFloat(thyroid['T3']);
      const t4 = parseFloat(thyroid['T4']);
      
      if (tsh > 4.20) {
        findings.push('⚠️ Elevated TSH - Hypothyroidism suspected');
      } else if (tsh < 0.27) {
        findings.push('⚠️ Low TSH - Hyperthyroidism suspected');
      }
    }
    
    // Check for multiple infections (coinfection alert)
    const infectionCount = findings.filter(f => 
      f.includes('malaria') || f.includes('typhoid') || f.includes('Brucella') || 
      f.includes('HIV') || f.includes('Hepatitis') || f.includes('H. Pylori')
    ).length;
    
    if (infectionCount > 1) {
      findings.unshift('🚨 MULTIPLE INFECTIONS DETECTED - Requires immediate comprehensive treatment plan');
    }
...
export default function Laboratory() {
  ...
  const handleLabTestSelect = (labTest: LabTest) => {
    ...
    // Create a clean technical summary of test results for lab technicians
    let readableSummary = "";
    if (labTest.results) {
      try {
        ...
        readableSummary = resultSummary.length > 0 ? resultSummary.join('\n\n') : "No test results recorded";
      } catch (e) {
