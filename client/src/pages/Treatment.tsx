...
import ResultDrawer from "@/components/ResultDrawer"; // ✅ NEW
...
export default function Treatment() {
  ...
  // ✅ unified drawer state for Lab/Xray/Ultrasound results
  ...
  const { data: todayEncounter } = useQuery<Encounter | null>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/encounters?date=${today}&patientId=${selectedPatient.patientId}`);
      if (!response.ok) return null;
  ...
  const addConsultationMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/encounters"] });
      toast({ title: "Consultation Added", description: "Consultation fee has been added to the patient's visit." });
    }
  });
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          ...
          <meta charset="utf-8">
          <style>
            @media print { body { margin: 0; } .prescription-container{ width:210mm; min-height:297mm; max-height:297mm; padding:20mm; box-sizing:border-box; page-break-after:avoid; display:flex; flex-direction:column; } .content{ flex:1; } .footer{ margin-top:auto; } }
        ...
        <body><div class="prescription-container">${prescriptionContent}</div></body>
      </html>
    `);
  ...
  const createTreatmentMutation = useMutation({
    ...
    onSuccess: (treatment: Treatment) => {
      ...
      toast({ title: "Success", description: `Treatment record saved successfully (ID: ${treatment.treatmentId})` });
    ...
    onError: (error: any) => {
      if (!navigator.onLine) {
        ...
        toast({ title: "Saved Offline", description: "Treatment record saved locally. Will sync when online." });
        setSavedTreatment(null);
        form.reset();
        setSelectedPatient(null);
      } else {
        toast({ title: "Error", description: "Failed to save treatment record", variant: "destructive" });
      }
    }
  });

  // Acknowledgment & Add-to-cart mutations
  const acknowledgeMutation = useMutation({
    mutationFn: async ({ orderLineId, acknowledgedBy, acknowledged }: { orderLineId: number; acknowledgedBy: string; acknowledged: boolean }) => {
      const response = await apiRequest("PUT", `/api/order-lines/${orderLineId}/acknowledge`, { acknowledgedBy, acknowledged });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/visits", currentEncounter?.encounterId, "orders"] });
      toast({ title: "Success", description: "Result acknowledgment updated" });
    },
  ...
  const addToCartMutation = useMutation({
    mutationFn: async ({ orderLineId, addToCart }: { orderLineId: number; addToCart: boolean }) => {
      const response = await apiRequest("PUT", `/api/order-lines/${orderLineId}/add-to-cart`, { addToCart });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/visits", currentEncounter?.encounterId, "orders"] });
      toast({ title: "Success", description: "Added to visit cart" });
    },
    onError: () => toast({ title: "Error", description: "Failed to add to cart", variant: "destructive" })
  });

  // Submit/edit/cancel pharmacy orders (unchanged)
  const submitMedicationsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setMedications([]);
      toast({ title: "Medications Ordered", description: `${medications.length} medication(s) sent to pharmacy` });
    },
    onError: (error: any) =>
      toast({ title: "Error", description: error.message || "Failed to submit medications", variant: "destructive" })
  });

  const cancelPrescriptionMutation = useMutation({
    mutationFn: async (orderId: string) => {
      const response = await apiRequest("PATCH", `/api/pharmacy-orders/${orderId}`, { status: "cancelled" });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/pharmacy-orders"] });
      toast({ title: "Success", description: "Prescription cancelled" });
    },
  ...
  const closeVisitMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/encounters"] });
      toast({ title: "Success", description: "Visit closed successfully" });
      setSelectedPatient(null);
      form.reset();
    },
  ...
  const handleCloseVisit = () => {
    ...
    const hasDiagnosis = (persistedDiagnosis && persistedDiagnosis.trim() !== "") || (currentDiagnosis && currentDiagnosis.trim() !== "");
    if (!hasDiagnosis) {
      toast({ title: "Validation Error", description: "Please enter and save a diagnosis before closing the visit", variant: "destructive" });
      return;
    }
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <Card className={`${selectedPatient.allergies && selectedPatient.allergies.trim() ? "border-2 border-red-500 bg-red-50 dark:bg-red-900/10" : "border-gray-200"}`}>
                <CardHeader className="pb-3">
                  <CardTitle
                    className={`flex items-center gap-2 text-base ${selectedPatient.allergies && selectedPatient.allergies.trim() ? "text-red-700 dark:text-red-400" : ""}`}
                  >
                ...
                <CardContent>
                  {selectedPatient.allergies && selectedPatient.allergies.trim() ? (
                    <div className="bg-white dark:bg-red-950/50 p-3 rounded-lg border border-red-300 dark:border-red-700">
                      <p className="text-red-900 dark:text-red-200 font-medium whitespace-pre-line">{selectedPatient.allergies}</p>
                    </div>
                  ) : (
                ...
                <CardContent>
                  {selectedPatient.medicalHistory && selectedPatient.medicalHistory.trim() ? (
                    <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
                      <p className="text-gray-700 dark:text-gray-300 text-sm whitespace-pre-line">{selectedPatient.medicalHistory}</p>
                    </div>
                  ) : (
                      ...
                      {recentTreatments.map((treatment) => (
                        <div key={treatment.treatmentId} className="bg-gray-50 dark:bg-gray-800 p-2 rounded text-sm border-l-2 border-blue-400">
                          <div className="flex items-center justify-between mb-1">
                            <span className="font-medium text-gray-900 dark:text-white">{new Date(treatment.visitDate).toLocaleDateString()}</span>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {labTests.map((test: any) => {
                          const testNames = Array.isArray(test.tests) ? test.tests : typeof test.tests === "string" ? JSON.parse(test.tests) : [];
                          return (
                            <div key={test.testId || test.orderId} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border-l-4 border-blue-500">
                              <div className="flex justify-between items-start gap-3">
                                <div className="flex-1">
                                  ...
                                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Requested: {new Date(test.requestedDate).toLocaleDateString()}</p>
                                ...
                                <div className="flex flex-col gap-2">
                                  {test.status === "completed" && test.orderLine && (
                                    <>
                                      ...
                                      {test.orderLine.addToCart === 1 && <Badge variant="outline" className="bg-green-50">Added</Badge>}
                                    </>
                                  )}
                                  {test.status === "completed" && (
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() => openResult("lab", test)} // ✅ drawer
                          ...
                          <div key={xray.examId || xray.orderId} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                            <div className="flex justify-between items-start gap-3">
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-400">{new Date(xray.requestDate).toLocaleDateString()}</p>
                                <Badge variant={xray.status === "completed" ? "default" : "secondary"} className="mt-1">
                                  {xray.status}
                                </Badge>
                                {xray.orderLine?.acknowledgedBy && (
                                  <p className="text-xs text-green-600 dark:text-green-400 mt-1">✓ Acknowledged by {xray.orderLine.acknowledgedBy}</p>
                                )}
                              </div>
                              <div className="flex flex-col gap-2">
                                  ...
                                  <>
                                    ...
                                    {xray.orderLine.acknowledgedBy && !xray.orderLine.addToCart && (
                                      <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => addToCartMutation.mutate({ orderLineId: xray.orderLine.id, addToCart: true })}
                                    ...
                                    {xray.orderLine.addToCart === 1 && <Badge variant="outline" className="bg-green-50">Added</Badge>}
                                  </>
                                )}
                                {xray.status === "completed" && (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => openResult("xray", xray)} // ✅ drawer
                          ...
                          <div key={ultrasound.examId || ultrasound.orderId} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                            <div className="flex justify-between items-start gap-3">
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-400">{new Date(ultrasound.requestDate).toLocaleDateString()}</p>
                                <Badge variant={ultrasound.status === "completed" ? "default" : "secondary"} className="mt-1">
                                  {ultrasound.status}
                                </Badge>
                                {ultrasound.orderLine?.acknowledgedBy && (
                                  <p className="text-xs text-green-600 dark:text-green-400 mt-1">✓ Acknowledged by {ultrasound.orderLine.acknowledgedBy}</p>
                                )}
                              </div>
                              <div className="flex flex-col gap-2">
                                  ...
                                  <>
                                    ...
                                    {ultrasound.orderLine.acknowledgedBy && !ultrasound.orderLine.addToCart && (
                                      <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => addToCartMutation.mutate({ orderLineId: ultrasound.orderLine.id, addToCart: true })}
                                    ...
                                    {ultrasound.orderLine.addToCart === 1 && <Badge variant="outline" className="bg-green-50">Added</Badge>}
                                  </>
                                )}
                                {ultrasound.status === "completed" && (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => openResult("ultrasound", ultrasound)} // ✅ drawer
            ...
            <Card className="border-l-4 border-l-green-500">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary">{orders.filter((o) => o.addToCart).length} items</Badge>
                          ...
                          .map((order: any) => (
                            <div key={order.orderId} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                              ...
                              <div className="text-right">
                                <Button variant="ghost" size="sm" onClick={() => addToCartMutation.mutate({ orderLineId: order.orderId, addToCart: false })}>
                                  Remove
                                </Button>
                              </div>
                            </div>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <Form {...form}>
                  <form onSubmit={handleSubmit} className="mt-6">
                    {/* Notes tab ... (unchanged content) */}
                    {/* --- keep your existing Notes/Imaging/Medications tab bodies here as-is --- */}
                    {/* I left all your tab content unchanged for brevity – this section is identical to your current file */}
                    {/* BEGIN: Notes */}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea placeholder="Detailed examination findings..." rows={4} {...field} value={field.value ?? ""} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea placeholder="Primary and secondary diagnoses..." rows={3} {...field} value={field.value ?? ""} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea placeholder="Medications, procedures, recommendations..." rows={4} {...field} value={field.value ?? ""} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    {/* Tests tab header + helper (unchanged) */}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                            ...
                            <a href="/laboratory" className="text-blue-600 hover:underline">
                              Laboratory
                            </a>{" "}
                            page.
                          </p>
                    ...
                    {/* Medications tab (unchanged content) */}
                    {/* ... keep your existing Medications tab code here (unchanged) ... */}
                    {/* I left this section identical to your current implementation */}
      ...
      {/* Edit Prescription Dialog (unchanged) */}
      ...
      {/* ✅ NEW: One universal Results Drawer (replaces 3 separate modals) */}
      <ResultDrawer
        ...
        onAddToSummary={(orderLineId: number, add: boolean) => addToCartMutation.mutate({ orderLineId, addToCart: add })}
      />

      {/* Today's Queue Modal (unchanged) */}
